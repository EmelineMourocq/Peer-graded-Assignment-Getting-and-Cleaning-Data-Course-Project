...
colnames(SubsetedData2)<-tolower(colnames(SubsetedData2))
        # remove duplicate bodybody in the variable names
colnames(SubsetedData2)<-sub("bodybody","body",names(SubsetedData2))
str(SubsetedData2)

# 5. From the data set in step 4, creates a second, independent tidy data set ####
# with the average of each variable for each activity and each subject. =T)
        # 1: group the dataframe by activity and subject, 2: get the mean per activity for each subject
library(dplyr)
MeanData<-SubsetedData2 %>%
        group_by(paste(SubsetedData2$activityname,SubsetedData2$subjectid)) %>%
        summarize_at(vars(4:82),funs(mean))
str(MeanData)
        #  separate pasted Activity and subject ID column into 2 columns (activityname and Subjectid)
library(tidyr)
MeanData2<-separate(MeanData,col="paste(SubsetedData2$activityname, SubsetedData2$subjectid)",into=c("activityname","subjectid"),sep=" ")
str(MeanData2)
View(MeanData2)

         # save MeanData2 into a txt file
write.table(MeanData2,"TidyData.txt",row.name=FALSE)


test<-read.table("TidyData.txt",sep=" ",header=T,fill=T)
str(test)
